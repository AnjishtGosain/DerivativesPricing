...
#include "BlackScholesDoubleNormalJump.h"
...
models::BlackScholesDoubleNormalJump::BlackScholesDoubleNormalJump(const double& costOfCarry, const double& discountRate, 
	const double& impliedVolatility, const double& initialUnderlyingPrice, const enumerations::UnderlyingCode& underlyingCode, 
	const double& dividendTime, const double& dividendAmount, const double& jumpTime, const double& jumpMean1, const double& jumpVolatility1,
	const double& jumpMean2, const double& jumpVolatility2, const double& bernoulliProbability)
{
	...
	setJumpMean1(jumpMean1);
	setJumpVolatility1(jumpVolatility1);
	setJumpMean2(jumpMean2);
	setJumpVolatility2(jumpVolatility2);
	setBernoulliProbability(bernoulliProbability);
}

void models::BlackScholesDoubleNormalJump::setImpliedVolatility(const double& value)
{
	if (value < -0.00000001)
...
void models::BlackScholesDoubleNormalJump::setJumpVolatility1(const double& value)
{
	if (value < -0.00000001)
		throw invalid_argument("The jump volatility 1 must be positive.");
	m_jumpVolatility1 = value;
}

void models::BlackScholesDoubleNormalJump::setJumpVolatility2(const double& value)
{
	if (value < -0.00000001)
		throw invalid_argument("The jump volatility 2 must be positive.");
	m_jumpVolatility2 = value;
}

void models::BlackScholesDoubleNormalJump::setBernoulliProbability(const double& value)
{
	if (value < -0.00000001 || value > 1.0000001)
		throw invalid_argument("The Bernoulli probability must be within 0 and 1.");
	m_bernoulliProbability = value;
}



void models::BlackScholesDoubleNormalJump::setInitialUnderlyingPrice(const double& value)
{
	if (value < -0.00000001)
...
void models::BlackScholesDoubleNormalJump::setDividendTime(const double& value)
{
	if (value < -0.00000001)
...
void models::BlackScholesDoubleNormalJump::setJumpTime(const double& value)
{
	if (value < -0.00000001)
...
void models::BlackScholesDoubleNormalJump::setDividendAmount(const double& value)
{
	if (value < -0.00000001)
		throw invalid_argument("The initial underlying price must be positive.");
...
const std::shared_ptr<vector<double>> models::BlackScholesDoubleNormalJump::generateMonteCarloSimulations(const int& nPaths, const double& time, 
	int seed)
{
	...
	bernoulli_distribution bernoulliDistribution{ m_bernoulliProbability };
	...
	for (int i = 0; i < nPaths; i++)
	{
		...
		if (m_dividendTime < time + 0.00000001 && m_jumpTime < m_dividendTime + 0.00000001)
		{
			...
			timeStep = m_jumpTime;
			if(bernoulliDistribution(mersenneTwisterEngine) == true)
				simulatedJump = m_jumpMean1 + m_jumpVolatility1 * normalDistribution(mersenneTwisterEngine);
			else
				simulatedJump = m_jumpMean2 + m_jumpVolatility2 * normalDistribution(mersenneTwisterEngine);
...
const std::shared_ptr<models::Tree> models::BlackScholesDoubleNormalJump::constructTree(const int& nTimeSteps, const double& timeToExpiry, 
	const enumerations::Implementation implementation, const double upperLimitStandardDeviation, const double lowerLimitStandardDeviation)
{
	...
	shared_ptr<vector<double>> jumpStatesPtr1, jumpProbabilitiesPtr1;
	tie(jumpStatesPtr1, jumpProbabilitiesPtr1) = LogNormalDiffusionTreeHelper::calculateNormalJumpStatesAndProbabilities(m_jumpMean1,
		m_jumpVolatility1, m_jumpTime, timeToExpiry);

	shared_ptr<vector<double>> jumpStatesPtr2, jumpProbabilitiesPtr2;
	tie(jumpStatesPtr2, jumpProbabilitiesPtr2) = LogNormalDiffusionTreeHelper::calculateNormalJumpStatesAndProbabilities(m_jumpMean2,
		m_jumpVolatility2, m_jumpTime, timeToExpiry);

	vector<double> jumpStates;
	vector<double> jumpProbabilities;
	for (int j = 0; j < jumpStatesPtr1->size(); j++)
	{
		jumpStates.push_back(jumpStatesPtr1->at(j));
		jumpProbabilities.push_back(m_bernoulliProbability * jumpProbabilitiesPtr1->at(j));
	}
	for (int j = 0; j < jumpStatesPtr2->size(); j++)
	{
		jumpStates.push_back(jumpStatesPtr2->at(j));
		jumpProbabilities.push_back((1.0 - m_bernoulliProbability) * jumpProbabilitiesPtr2->at(j));
	}
...
const std::shared_ptr<double> models::BlackScholesDoubleNormalJump::smoothedValueAtTreeNode(const double underlyingPrice, 
...
const bool models::BlackScholesDoubleNormalJump::supportsVanillaOptionSmoothing(const double & timeStart, const double & timeEnd)
{
